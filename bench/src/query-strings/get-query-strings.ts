...
export async function getQueryStrings(queries_dir: Path): Promise<[string, string][]> {
    const queries = (await getContentsOfFilesInDir(queries_dir, (filePath) => filePath.endsWith(".sparql"))).sort((a, b) => a[0].localeCompare(b[0]));
    return queries.map(([filePath, queryS]) => [path.basename(filePath).replace(".sparql", ""), queryS]);
}

export async function getQueryStrings2(templates_dir: Path, substitutions_dir: Path): Promise<[string, string][]> {
    ...
    const ret: Awaited<ReturnType<typeof getQueryStrings>> = [];
